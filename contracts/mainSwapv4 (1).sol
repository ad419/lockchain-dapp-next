...
contract LockSwap is Ownable, ReentrancyGuard {
    ...
    function createBuy(address _referral) external payable nonReentrant {
        ...
        uint commission = 0; 
        bool eligibleForCommission;
        uint256 currentCommission = tokenCommission(LockSwapToken).referralCommision();
        if(currentCommission > 0) commission = overall.mul(currentCommission).div(100);
        ...
        if(commission > 0) {
            if(eligibleForCommission) {
                ...
                IERC20(LockSwapToken).transfer(address(LockSwapToken),commission);   
            }
        }
